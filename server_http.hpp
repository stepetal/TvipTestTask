...
namespace SimpleWeb {
  ...
  template <class socket_type>
  class SnapshotServer : public ServerBase<socket_type> {};
  
  template <>
  class SnapshotServer<HTTP> : public ServerBase<HTTP> {
    std::vector<std::string> file_names;
    std::string video_files_dir;
    std::string thumbs_dir;
    std::unique_ptr<ffmpegthumbnailer::VideoThumbnailer> video_thumbnailer;
    uint16_t max_thumb_width;
    uint16_t max_thumb_height;
    std::string video_format;
    
  public:
    /// Constructs a server object.
    SnapshotServer() noexcept : ServerBase<HTTP>::ServerBase(80), video_files_dir(std::filesystem::current_path().string()), 
                                                                  video_thumbnailer(std::make_unique<ffmpegthumbnailer::VideoThumbnailer>()),
                                                                  max_thumb_height(1024),
                                                                  max_thumb_width(1024),
                                                                  video_format("mp4") {}
                                                                  
    void set_video_files_dir(const std::string& dir_name) { video_files_dir = dir_name; }
    void set_thumbs_dir(const std::string& dir_name) { thumbs_dir = dir_name; }
    void set_max_thumb_width(uint16_t max_w){max_thumb_width = max_w;}
    void set_max_thumb_height(uint16_t max_h){max_thumb_height = max_h;}
    void set_video_format(const std::string& v_format){video_format = v_format;}
    
    void init()
    {
        resource[".*"]["GET"] = [&](std::shared_ptr<Response> response, std::shared_ptr<Request> request) {
        try {
            auto error_map = status_code_strings();
            std::regex file_regex("/file([0-9]+)/([0-9]+)x([0-9]+)/([0-9]{1}[0-9]?)\\.(jpg|png)");
            std::smatch matches;
            std::string resource_line = request->path_match[0].str();
            if(std::regex_search(resource_line,matches,file_regex))
            {
                bool found{false};
                if(!std::filesystem::is_empty(video_files_dir))
                {
                    for(const auto& entry : std::filesystem::directory_iterator(video_files_dir))
                    {
                        auto file_number = matches[1].str();
                        if((entry.path().string().find(std::string("file") + file_number) != std::string::npos))
                        {
                            std::regex video_format_regex(".*\\.(.*)");
                            std::smatch video_format_match;
                            std::string video_file_name = entry.path().string();
                            if(std::regex_search(video_file_name,video_format_match,video_format_regex))
                            {
                                if(video_format_match[1].str() != video_format)
                                {
                                    response->write(StatusCode::client_error_unprocessable_entity, error_map[StatusCode::client_error_unprocessable_entity]);
                                    throw std::invalid_argument(error_map[StatusCode::client_error_unprocessable_entity]);
                                }
                            }
                            else
                            {
                            ...
                            found = true;
                            auto image_width = matches[2].str();
                            auto image_height = matches[3].str();
                            auto time_to_seek_in_percent = matches[4].str();
                            auto image_format = matches[5].str();
                            
                            if((std::stoi(time_to_seek_in_percent) > 100))
                            {
                                response->write(StatusCode::client_error_unprocessable_entity, error_map[StatusCode::client_error_unprocessable_entity]);
                                throw std::invalid_argument(error_map[StatusCode::client_error_unprocessable_entity]);
                            }
                            ...
                            {
                                video_thumbnailer->setSeekPercentage(std::stoi(time_to_seek_in_percent));
                            }
                            if((std::stoi(image_width) > max_thumb_width) || (std::stoi(image_height) > max_thumb_height))
                            {
                                response->write(StatusCode::client_error_unprocessable_entity, error_map[StatusCode::client_error_unprocessable_entity]);
                                throw std::invalid_argument(error_map[StatusCode::client_error_unprocessable_entity]);
                            }
                            ...
                            {
                                if((std::stoi(image_width) == 0) || (std::stoi(image_height) == 0))
                                {
                                    video_thumbnailer->setThumbnailSize(0,0); // original size
                                }
                                else
                                {
                                    video_thumbnailer->setThumbnailSize(std::stoi(image_width),std::stoi(image_height));
                                }
                            }
                            
                            if(image_format == "jpg")
                            {
                                video_thumbnailer->generateThumbnail(entry.path().string(), Jpeg, thumbs_dir + std::string("file") + file_number + "." + image_format);
                            }
                            if(image_format == "png")
                            {
                                video_thumbnailer->generateThumbnail(entry.path().string(), Png, thumbs_dir + std::string("file") + file_number + "." + image_format);
                            }
                            
                            // Case 1
                            // this approach does not work: no image on the screen
                            //std::vector<uint8_t> buffer;
                            //video_thumbnailer->generateThumbnail(entry.path().string(), Jpeg, buffer);
                            
                            /*
                            CaseInsensitiveMultimap header;
                            header.emplace("Content-Length", std::to_string(buffer.size()));
                            ...
                            */
                            ...
                            //Case 2
                            // This approach does not work: src address is not correct
                            //stream << std::string("<img src=\"") + thumbs_dir + std::string("file") + request->path_match[1].str() + "." + request->path_match[3].str() + "\">";
                            //response->write(stream);
                            
                            // this approach works fine, but we need to open file again
                            // Case 1 and case 2 are the best, but no idea why they didn't work...
                            // see http_examples.hpp in SimpleWebServer project
                            // write image on the screen in raw bytes
                            CaseInsensitiveMultimap header;
                            auto ifs = std::make_shared<std::ifstream>();
                            ifs->open(thumbs_dir + std::string("file") + file_number + "." + image_format, std::ifstream::in | std::ios::binary | std::ios::ate);
                            if(*ifs) 
                            {
                                ...
                                class FileServer {
                                public:
                                  static void read_and_send(const std::shared_ptr<Response> &response, const std::shared_ptr<std::ifstream> &ifs) {
                                    ...
                                    if((read_length = ifs->read(&buffer[0], static_cast<std::streamsize>(buffer.size())).gcount()) > 0) {
                                      ...
                                      if(read_length == static_cast<std::streamsize>(buffer.size())) {
                                        response->send([response, ifs](const error_code &ec) {
                                          if(!ec)
                                            read_and_send(response, ifs);
                                          else
                                            response->write(StatusCode::client_error_request_timeout, "Connection interrupted");
                                        });
                                      }
                                    }
                                  }
                ...
                if(!found)
                {
                    response->write(StatusCode::client_error_not_found, error_map[StatusCode::client_error_not_found]);
                    throw std::invalid_argument(error_map[StatusCode::client_error_not_found]);
                }
            }
            else
            {
                response->write(StatusCode::client_error_bad_request, error_map[StatusCode::client_error_bad_request]);
                throw std::invalid_argument(error_map[StatusCode::client_error_bad_request]);
            }
        }
        catch(const std::exception &e) {
            // no idea, why it is not displayed in the browser
            /*
            std::stringstream stream;
            auto error_map = status_code_strings();
            stream << "<h1>" << error_map[StatusCode::client_error_not_found] << "</h1>";
            response->write(stream);
            */
        }
      };
